...
var memResize = memSVG.append("image")
    ...
    .on("click", function(){
        ...
        if(memChartIsFullScreen) {
            d3.select(".memChart .maximize").attr("xlink:href","images/minimize_24_grey.png")
            // Redraw this chart only
            resizeMemChart();
        } else {
            canvasWidth = $("#memDiv1").width() - 8; // -8 for margins and borders
            graphWidth = canvasWidth - margin.left - margin.right;
            d3.select(".memChart .maximize").attr("xlink:href","images/maximize_24_grey.png")
            canvasHeight = 250;
    ...
    .on("mouseover", function() {
        if(memChartIsFullScreen) {
            d3.select(".memChart .maximize").attr("xlink:href","images/minimize_24.png")
        } else {
            d3.select(".memChart .maximize").attr("xlink:href","images/maximize_24.png")
        }
    })
...
function resizeMemChart() {
 ...
 memChartPlaceholder
  ...
  memResize.attr("x", canvasWidth - 30)
      .attr("y", 4)
