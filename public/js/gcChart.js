...
var gcResize = gcSVG.append("image")
    ...
    .on("click", function(){
        ...
        if(gcChartIsFullScreen) {
            d3.select(".gcChart .maximize").attr("xlink:href","images/minimize_24_grey.png")
            // Redraw this chart only
            resizeGCChart();
        } else {
            canvasWidth = $("#gcDiv").width() - 8; // -8 for margins and borders
            graphWidth = canvasWidth - margin.left - margin.right;
            d3.select(".gcChart .maximize").attr("xlink:href","images/maximize_24_grey.png")
            canvasHeight = 250;
    ...
    .on("mouseover", function() {
        if(gcChartIsFullScreen) {
            d3.select(".gcChart .maximize").attr("xlink:href","images/minimize_24.png")
        } else {
            d3.select(".gcChart .maximize").attr("xlink:href","images/maximize_24.png")
        }
    })
...
function resizeGCChart() {
  if(gcChartIsFullScreen) {
    canvasWidth = $("#gcDiv").width() - 30; // -30 for margins and borders
    graphWidth = canvasWidth - margin.left - margin.right;
